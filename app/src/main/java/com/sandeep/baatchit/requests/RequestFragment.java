...
import com.sandeep.baatchit.requests.RequestModel;
...
public class RequestFragment extends Fragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        databaseReferenceRequests = FirebaseDatabase.getInstance().getReference().child(NodeName.FRIEND_REQUESTS).child(currentUser.getUid());
        ...
        databaseReferenceRequests.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                for(DataSnapshot ds : dataSnapshot.getChildren()){
                    if(ds.exists()){
                        String requestType = ds.child(NodeName.REQUEST_TYPE).getValue().toString();
                        if(requestType.equals(Constants.REQUEST_STATUS_RECEIVED))
                        {
                            ...
                            databaseReferenceUsers.child(userId).addListenerForSingleValueEvent(new ValueEventListener() {
                                @Override
                                public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                                    String userName = dataSnapshot.child(NodeName.NAME).getValue().toString();

                                    String photoName="";
                                    if(dataSnapshot.child(NodeName.PHOTO).getValue()!=null)
                                    {
                                        photoName = dataSnapshot.child(NodeName.PHOTO).getValue().toString();
                                    }
